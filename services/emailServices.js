...
module.exports = class EmailController {
  ...
  register(to) {
    try {
      ...
      var content = "Merci d'avoir créé votre compte chez nous";
  ...
  resetEmailPassword(to, token) {
    try {
      var object = "Reset Password Link - example.com";
      var content =
        'You requested for reset password, kindly use this <a href="https://localhost:8080/reset-password?token=' +
        token +
        '">Reset password</a> to reset your password';
      this.sendMyEmail(to, object, content);
    } catch (err) {
      console.log(err);
      return res.status(500).json(err);
